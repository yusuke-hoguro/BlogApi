...
func validateCommentInput(comment models.Comment, postID int) *apperror.AppError {
	...
	if strings.TrimSpace(comment.Content) == "" {
		return apperror.NewAppError(apperror.TypeBadRequest, fmt.Sprintf("Content is required : PostID=%d", postID), nil)
	}

	// コメントが指定文字以上の場合はエラーとする
	if len(comment.Content) > MaxCommentLength {
		return apperror.NewAppError(apperror.TypeBadRequest, fmt.Sprintf("Content must be %d characters or less : PostID=%d", MaxCommentLength, postID), nil)
	}
