...
test('新規投稿作成→編集→削除のテストを実施', async ({ page }) => {
    ...
    // タイトルが変わったか確認
    await expect(page.getByLabel('タイトル')).toHaveValue(updateTitle, { timeout: 5000 })
    // 投稿内容を編集する
    const updateContext = `E2E 投稿本文です。更新しました。`;
    await page.getByLabel('内容').fill(updateContext);
    // 投稿内容が変わったか確認
    await expect(page.getByLabel('内容')).toHaveValue(updateContext, { timeout: 5000 })
    // 更新ボタンを押下する
    await page.getByRole('button', { name: '更新' }).click();
    // 投稿のタイトルが表示されていることを確認する
    await expect(page.getByTestId('post-title')).toContainText(updateTitle, { timeout: 5000 })
    // 投稿の内容が表示されていることを確認する
    await expect(page.getByTestId('post-content')).toContainText(updateContext, { timeout: 5000 })
